...
<div id="content">
	<div id="page-content">
		<div id="page-header">Remove loan transaction</div>
		<div id="sidebar">
			<span class="msgbox">
				<span class="tips"><span style="color: #fc661c">Tips!</span><br/>You may remove or delete a record here.</span>
			</span>
		</div>
		<p>In this section, you can virtually remove a loan by setting it transaction status to "Void".<br/>
...
<?php
	if ( isset( $_POST['submit'] ) ){
		if ( isset( $_POST['id'] ) ){			require_once $GLOBALS["classRoot"].'LibraryDb.php';
			...
			if ( $myLibrary->connect() )  {
				...
				$thisLoan = new Loan();
				$thisLoan->setLoanId( $_POST['id'] );
				if ( $_POST['submit'] == 'Confirm Remove' ){
					$execution = $myLibrary->removeRec($thisLoan->getTableName(), 'status', $thisLoan->getIdName(), $thisLoan->getLoanId());
					if ( $execution ){
					...
					$execution = $myLibrary->deleteRec($thisLoan->getTableName(), $thisLoan->getIdName(), $thisLoan->getLoanId());
					if ( $execution ){
	...
	} elseif ( isset( $_GET['id'] ) ){		// print the loan record relevant to the book id.
		require_once $GLOBALS["classRoot"].'LibraryDb.php';
		$myLibrary = new LibraryDb();
		if (  $myLibrary->connect() )  {
			require_once $GLOBALS["classRoot"].'Loans.php';
			$thisLoan = new Loan();
			$thisLoan->setLoanId( $_GET[id] );
			$record = $myLibrary->getRecords($thisLoan->getTableName(), $thisLoan->getIdName(), $thisLoan->getLoanId() );
			$searchResult = $thisLoan->getTable( $record );
